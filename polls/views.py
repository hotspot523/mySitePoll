...
def post_new(request):
    ...
    if request.method == "POST":
        form = QuestionForm(request.POST, initial={'data': json})

        # print abcd,"poppppppppppppppppppppppppppp"
        print(form)
        ...
        if form.is_valid():
            ...
            print(type(post),"111111111111111111111111111111111111")
            ...
            #return render(request, 'polls/post_detail.html', {'pk': post.pk})
            #return redirect('post_detail', pk=post.pk)
            #reverse('affiche_all', devise=('EURO',))
...
def show_detail(request):
    ...
    return render(request, 'polls/all_detail.html', {'question': question})

def show_q_detail(request, pk):

    #question = get_object_or_404(question)

    q = Question.objects.filter(id = pk).values('question_text')

    question = Question.objects.filter(question_text = q)
